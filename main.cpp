...
string** updateBoard(string** values, int input, string character)
{
    switch (input) {
        case 1:
            values[0][0] = character;
            break;
        case 2:
            values[0][1] = character;
            break;
        case 3:
            values[0][2] = character;
            break;
        case 4:
            values[1][0] = character;
            break;
        case 5:
            values[1][1] = character;
            break;
        case 6:
            values[1][2] = character;
            break;
        case 7:
            values[2][0] = character;
            break;
        case 8:
            values[2][1] = character;
            break;
        case 9:
            values[2][2] = character;
            break;
    }
...
{
    bool isValid = false;
    float validatedInput;
    string curBoardLocation;
    do {
        cout << "   Please enter a valid input that is a whole number (1-9) that is not taken:" << endl;
        cin >> validatedInput;
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');

        if (validatedInput && validatedInput >= 1 && validatedInput <= 9 && floor(validatedInput) == validatedInput && validatedInput == ceil(validatedInput))
        {
            int vInput = static_cast<int>(round(validatedInput));
            switch (vInput) {
